...
import { GraduationCap, Clock, Layers, BookOpen, ArrowRight } from "lucide-react";
      ...
      {enrollments.length > 0 && (
        <SectionCard title="Continue learning" description="Pick up where you left off.">
          <ul className="space-y-4">
            {enrollments.map((e) => (
              <li key={e.id}>
                <Link href={`/training/${e.courseId}`} className="group flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    <p className="truncate text-sm font-medium text-slate-800 group-hover:text-brand-700">
                      {e.course.title}
                    </p>
                    <p className="text-xs text-slate-400">
                      {e.progressPct}% · {e.status === "COMPLETED" ? "Completed" : "In progress"}
                    </p>
                  </div>
                  <span className="inline-flex items-center gap-1 text-sm font-medium text-brand-600">
                    {e.status === "COMPLETED" ? "Review" : "Continue"} <ArrowRight className="h-4 w-4" />
                  </span>
                </Link>
