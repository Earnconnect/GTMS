...
export default function NewTicketButton() {
  ...
  useEffect(() => {
    if (state.success && state.ticketId) {
      router.push(`/business/support/${state.ticketId}`);
    }
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
            ...
            <form action={action} className="space-y-4">
              ...
              <FormField label="Message" required>
                <Textarea name="body" rows={4} placeholder="Describe your issue..." required />
              </FormField>
              <div className="flex gap-3">
                <Button type="submit" className="flex-1" loading={isPending}>Submit</Button>
